...
def generate_comprehensive_report(output_file: str = "reports/validation_report.md"):
    ...
    try:
        ...
        if std < 2.0:
            ...
            interpretation = "⚠️ **Moderate** - Performance depends on time period"
        else:
        ...
        print(f"⚠️  K-Fold validation failed: {e}")
        report.append("\n## 1. K-Fold Cross-Validation")
        report.append(f"\n❌ **Failed:** {str(e)}")
    ...
    try:
        ...
        if bootstrap_results['cross_regional']:
            ...
            if cr['n_samples'] < 50:
                report.append(f"\n⚠️ **Warning:** Small sample size - need more international tournament data")
            elif cr['n_samples'] < 100:
                report.append(f"\n⚠️ **Note:** Moderate sample size - margin of error is significant")
            else:
                report.append(f"\n✅ **Good:** Sufficient sample size for reliable estimates")

    except Exception as e:
        print(f"⚠️  Bootstrap CI failed: {e}")
        report.append("\n\n## 2. Bootstrap Confidence Intervals")
        report.append(f"\n❌ **Failed:** {str(e)}")
